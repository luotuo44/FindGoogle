...
std::string getDNSPacket(const std::string &hostname)
{
    char buff[1024];
    ...
    question_type->qtype = SocketOps::htons(1);
    question_type->qclass = SocketOps::htons(1);
                           ...
                           int *count, int *type)
{
    ...
    int num = sizeof(ANSWER_t) + SocketOps::ntohs(answer->data_len);


    std::string name;

    *type = SocketOps::ntohs(answer->qtype);
    ...
    {
        name = SocketOps::parseIP(answer->data);
    }
    else //其他类型不进行解析
    {
        name = "unknown result";
        ...
        parseDNSResultPacket(const unsigned char *buff, int len)
{
    ...
    int answer_num = SocketOps::ntohs(head->a_count);
